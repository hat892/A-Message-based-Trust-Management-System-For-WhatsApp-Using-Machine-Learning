...
public class SimulatorOutput{
	...
	public void printStatistics(Globals GLOBALS, Statistics STATS){
		...
		out.printf(">Good User Failures:     %d\n", STATS.NUM_GOOD_FAIL);
         ...
         public void writeResultsCSV(Globals GLOBALS, Statistics STATS,SimulatorMalicious.MAL_STRATEGY STRATEGY,int type, double run_time, String TA) {
                int INCMPLETE_TRANS = (STATS.NUM_RECV_BLK_TR + STATS.NUM_SEND_BLK_TR);
		int COMPLETED_TRANS = (GLOBALS.NUM_TRANS - INCMPLETE_TRANS);
		int NUM_VALID_TRANS = (COMPLETED_TRANS - STATS.NUM_INVAL_TRANS);
		int TOTAL_GOOD_TRANS = (STATS.NUM_GOOD_SUCC + STATS.NUM_GOOD_FAIL);
                ...
                StringBuilder buf = new StringBuilder();
                        buf.append(TA).append(",")
                        .append(STRATEGY).append(",")  
                        .append(malType).append(",")           
                        .append(GLOBALS.NUM_USERS).append(",")
                        .append(GLOBALS.NUM_TRANS).append(",")
			.append(GLOBALS.USR_DISG+GLOBALS.USR_PROV+GLOBALS.USR_FEED+GLOBALS.USR_PURE+GLOBALS.USR_SYBL).append(",")
			
                        .append(((double)STATS.NUM_GOOD_SUCC/TOTAL_GOOD_TRANS)*100).append(",")
                        .append(run_time).append("\r\n"); 
                    try {
