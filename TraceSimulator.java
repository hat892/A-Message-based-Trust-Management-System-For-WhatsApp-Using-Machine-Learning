...
public class TraceSimulator{
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.printf("Simulation runtime: %f secs\n\n", run_time);
			...
			// Print URL stats if WhatsTrust was used
		if(TSYSTEM == TSYS.WHATSTRUST){
			((WhatsTrustTM) TALG).printURLStats();
		}
		...
		for(int i=1; i < args.length; i+=2){
			if(args[i-1].equalsIgnoreCase("-input"))
				...
				if(args[i].equalsIgnoreCase("eigen"))
					...
					TSYSTEM = TSYS.TNA_SL;
				else if(args[i].equalsIgnoreCase("socialtrust"))
					TSYSTEM = TSYS.Socialtrust;
				else if(args[i].equalsIgnoreCase("Maimona"))
					TSYSTEM = TSYS.Maimona;
				else if(args[i].equalsIgnoreCase("whatstrust"))
					TSYSTEM = TSYS.WHATSTRUST;
				else
					TSYSTEM = TSYS.NONE;
